...
def dino_loss(student_output, teacher_output, student_temp=0.1, teacher_temp=0.04, center=0.0):
    ...
    """
    student_out = F.log_softmax((student_output - center) / student_temp, dim=-1)
    teacher_out = F.softmax((teacher_output - center) / teacher_temp, dim=-1)
...
class Trainer:
    ...
    def __init__(self, student_model, teacher_model, optimizer, scheduler, exp_name, device):
    ...
    def train_epoch(self, trainloader):
        ...
        for batch in trainloader:
            ...
            for teacher_param, student_param in zip(self.teacher_model.parameters(), self.student_model.parameters()):
                teacher_param.data.mul_(0.995)
                teacher_param.data += 0.005 * student_param.data
...
def main():
    ...
    trainer = Trainer(student_model, teacher_model, optimizer, scheduler, EXP_NAME, device=device)
